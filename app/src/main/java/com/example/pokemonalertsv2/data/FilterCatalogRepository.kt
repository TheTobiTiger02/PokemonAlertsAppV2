...
internal fun mergeFilterCatalog(cached: FilterCatalog?, localSpecies: List<String>, currentAlerts: List<PokemonAlert>): FilterCatalog {
        ...
        val observedQuests = currentAlerts.mapNotNull { alert ->
            ...
            FilterCatalogQuest("${pair.taskKey}::${pair.rewardKey}", pair.taskKey, pair.rewardKey,
                alert.questTask.orEmpty(), alert.questReward.orEmpty(), true)
        }
        return (cached ?: FilterCatalog()).copy(
            ...
            raidSpecies = (cached?.raidSpecies.orEmpty() + localSpecies + observedSpecies).distinctBy(::normalizeFilterToken),
